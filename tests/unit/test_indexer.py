...
class TestIndexer:
    ...
    def test_prepare_text_minimal(self, mock_qdrant_client, mock_embedding_model):
        ...
        with patch(
            ...
            with patch("core.model_loader.ModelLoader.get_model", return_value=mock_embedding_model):
                indexer = Indexer()
                text = indexer.prepare_text(Movie(id=1, title="Unknown"))

                assert "название unknown" in text.lower()
                assert "описание: фильм без описания" not in text.lower()

    def test_prepare_text_empty_contract(self, mock_qdrant_client, mock_embedding_model):
        """Movie contract should provide a safe fallback for an empty record."""
        with patch(
            "core.qdrant_client.QdrantClientSingleton.get_client",
            return_value=mock_qdrant_client,
        ):
            with patch("core.model_loader.ModelLoader.get_model", return_value=mock_embedding_model):
                indexer = Indexer()
                text = indexer.prepare_text(Movie(id=1))
    ...
    def test_normalize_movie_accepts_legacy_contract(
        ...
        """Should validate legacy movie keys through the movie contract."""
        with patch(
            "core.qdrant_client.QdrantClientSingleton.get_client",
            return_value=mock_qdrant_client,
        ):
    ...
    def test_load_movies(self, mock_qdrant_client, mock_embedding_model, temp_json_file):
        """Should load movies from JSON into movie contracts."""
        with patch(
            "core.qdrant_client.QdrantClientSingleton.get_client",
            return_value=mock_qdrant_client,
        ):
